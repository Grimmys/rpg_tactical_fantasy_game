...
class ItemButton(Button):
    ...
    def __init__(
        ...
        if item and not disabled:
            raw_item_frame_hover: pygame.Surface = pygame.image.load(
                ITEM_SPRITE
            ).convert_alpha()
            ...
            name_rendering_hover: pygame.Surface = fonts["ITEM_FONT_HOVER"].render(
                name, True, MIDNIGHT_BLUE
            )
            nb_lines = 3 if price_text or quantity_text else 2
            if price_text:
                price_rendering_hover = fonts["ITEM_FONT_HOVER"].render(
                    price_text, True, MIDNIGHT_BLUE
                )
                item_frame_hover.blit(
                    price_rendering_hover,
                    (
                        frame.get_width() + padding * 2,
                        2 * item_frame.get_height() / 3
            ...
            if quantity_text:
                quantity_rendering = fonts["ITEM_FONT_HOVER"].render(
                    quantity_text, True, MIDNIGHT_BLUE
                )
                item_frame_hover.blit(
                    quantity_rendering,
                    (
