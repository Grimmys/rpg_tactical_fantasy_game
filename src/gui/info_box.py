...
class InfoBox:
    ...
    def __init__(
        ...
        sprite: str,
        entries: Entries,
        id_type: Type[Enum] = None,
        width: int = DEFAULT_WIDTH,
        element_linked: pygame.Rect = None,
        close_button: Callable = None,
        ...
        self.elements: List[List[Union[BoxElement, int]]] = self.init_elements(
            self.entries, width
        )
        ...
        for row in entries:
            ...
            for entry in row:
                ...
                if "type" not in entry:
                    ...
                    element.append(
                        BoxElement((0, 0), pygame.Surface((0, 0)), entry["margin"])
                    )
                    ...
                    raw_inactive_button = pygame.image.load(
                        BUTTON_INACTIVE
                    ).convert_alpha()
                    base_sprite = pygame.transform.scale(
                        raw_inactive_button, BUTTON_SIZE
                    )
                    ...
                    base_sprite_hover = pygame.transform.scale(
                        raw_active_button, BUTTON_SIZE
                    )
                    sprite_hover = base_sprite_hover.copy()
                    sprite_hover.blit(
                        name,
                        (
                            base_sprite_hover.get_width() // 2 - name.get_width() // 2,
                    ...
                    name = fonts["ITEM_FONT"].render(
                        entry["name"], True, entry["color"]
                    )
                    name_hover = fonts["ITEM_FONT"].render(
                        entry["name"], True, entry["color_hover"]
                    )
                    if "obj" not in entry:
                        entry["obj"] = None
                    element.append(
    ...
    def determine_height(self, close_button: Callable) -> int:
        ...
        if close_button:
            close_button_height: int = (
                CLOSE_BUTTON_SIZE[1] + MARGIN_TOP + CLOSE_BUTTON_MARGIN_TOP
            )
    ...
    def display(self, screen: pygame.Surface) -> None:
        ...
        if self.separator["display"]:
            pygame.draw.line(
                ...
                (
                    self.position[0] + self.size[0] / 2,
                    self.position[1] + self.separator["height"],
                ),
                2,
            )
